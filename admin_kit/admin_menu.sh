  ...
  while true; do
    ...
    case "$choice" in
      1)
        ...
        bash "$SCRIPT_DIR/scripts/system/update_system.sh" || { echo_error "System update/upgrade failed."; exit 1; }
        pause
        ;;
      2)
        clear
        display_header "Create sudo user"
        bash "$SCRIPT_DIR/scripts/system/create_user.sh" || { echo_error "User creation failed."; exit 1; }
        pause
        ;;
      3)
        clear
        display_header "Setting timezone"
        bash "$SCRIPT_DIR/scripts/system/set_timezone.sh" || { echo_error "Setting timzone failed."; exit 1; }
        pause
        ;;
      4)
        clear
        display_header "Set hostname"
        bash "$SCRIPT_DIR/scripts/system/set_hostname.sh" || { echo_error "Setting hostname failed."; exit 1; }
        pause
        ;;
      0) return ;;
...
backups_screen(){
  ...
  #       echo_info "This feature is under development."
  #       pause ;;
  #     0) return ;;
  #     *) echo_error "Invalid option. Please try again."; pause ;;
  #   esac
...
developer_screen(){
  while true; do
    clear
    display_header "Develoepr tools"
    ...
    case "$choice" in
      ...
      3)
        ...
        run "$SCRIPT_DIR/scripts/developer/instal_composer.sh" || echo_error "Composer install failed."; pause ;;
      4)
        clear
        display_header "Install Node.js and npm"
        run "$SCRIPT_DIR/scripts/developer/instal_nodejs.sh" || echo_error "Node.js install failed."; pause ;;
      # 5)
      #   clear
...
#   while true; do
#     clear
#     display_header "Helper scripts"
#     #echo_note "1) Install extrepo (external apt repositories)"
#     echo_note "2) Install common tools (curl, wget, git, unzip, etc.) --in development"
#     echo_note "3) Install php and common extensions --in development"
#     echo_note "0) Return to Main Menu"
#     echo -ne "\n${YELLOW}Enter your choice:${NC} "
#     read -r choice
...
#       # 1)
#       #   clear
#       #   display_header "Helper Tools"
#       #   run "$SCRIPT_DIR/scripts/helper/install_extrepo.sh" || echo_error "Extrepo install failed."; pause;;
#       2)
#         clear
#         display_header "Install common tools"
#         run "$SCRIPT_DIR/scripts/helper/install_common_tools.sh" || echo_error "Common tools install failed."; pause;;
#       3)
#         clear
#         display_header "Environment (.env) manager"
#         run "$SCRIPT_DIR/scripts/helper/install_php.sh" || echo_error "Env manager failed."; pause;;
#       0) return ;;
#       *) echo_error "Invalid option. Please try again."; pause ;;
#     esac
#   done
