...
check_conflicts(){ ! apt_package_installed mysql-server && ! apt_package_installed mysql-community-server; }
...
main(){ echo_info "This installs MariaDB (server and client)."; echo_info "MariaDB conflicts with MySQL."; confirm "Do you want to continue?" || { echo_info "Cancelled."; exit 0; }; run_install_workflow "mariadb" check_installed check_conflicts install_step summary_step; }
main "$@"
